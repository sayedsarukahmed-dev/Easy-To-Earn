/* ============================================================
   ADS.JS
   Simulated rewarded-ad player. In the real native app this is
   where the AdMob Rewarded Ad SDK callback plugs in:
     - onAdCompleted()  -> only THIS path may call creditAdWatch()
     - onAdSkipped() / onAdFailedToShow() -> no credit, no count
   The rule "skip = no reward, no count" is enforced here at the
   player level, not trusted from any UI button state, so a user
   cannot get credit by any UI trick.
   ============================================================ */
...
function openAdPlayer(onDone){
  ...
  skipBtn.addEventListener("click", () => {
    ...
    if(remaining > 0){
      showSkipWarning(() => {
        // user confirmed skip mid-ad — NO credit, NO count
        clearInterval(tick);
        cleanup();
